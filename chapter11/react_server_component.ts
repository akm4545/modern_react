...

{
    ...
    // 서버 사이드 렌더링응의 목적은 초기의 인터랙션은 불가능하지만 HTML을 빠르게 내려주는 데 초점일 두고 있다
    // 따라서 여전히 초기 HTML이 로딩된 이후에는 클라이언트에서 자바스크립트 코드를 다운로드, 파싱, 실행에비용이 든다

    // 따라서 이후에는 서버 사이드 렌더링과 서버 컴포넌트를 모두 채택하는 것도 가능해질 것이다
    // 서버 컴포넌트를 활용해 서버에서 렌더링할 수 있는 컴포넌트는 서버에서 완성해서 제공받은 다음 
    // 클라이언트 컴포넌트는 서버 사이드 렌더링으로 초기 HTML으로 빠르게 전달받을 수 있다
    // 이 두 가지 방법을 모두 결합하면 클라이언트 및 서버 컴포넌트를 모두 빠르게 보여줄 수 있고 동시에 클라이언트에서 내려받아야 하는
    // 자바스크립트의 양도 줄어들어 브라우저의 부담을 덜 수도 있다

    // 결론적으로 둘은 대체제가 아닌 상호보완하는 개념으로 봐야 할 것이다
}

{
    ...
    // waitForWebpack은 단순히 개발 환경에서 웹팩이 빌드 경로에 index.html을 만들 때까지 기다리는 코드
    // 사용자가 최초에 들어왔을 때 수행하는 작업은 오로지 indext.html을 제공하는 것 뿐이다

    // 1. 서버가 렌더링 요청을 받는다 서버가 렌더링 과정을 수행해야 하므로 리액트 서버 컴포넌트를 사용하는 모든 페이지는 항상
    // 서버에서 시작된다 즉 루트에 있는 컴포넌트는 항상 서버 컴포넌트다

    // 2. 서버는 받은 요청에 따라 컴포넌트를 JSON으로 직렬화(serialize)한다 이때 서버에서 렌더링할 수 있는 것은 직렬화해서
    // 내보내고 클라이언트 컴포넌트로 표시된 부분은 해당 공간을 플레이스홀더 형식으로 비워두고 나타낸다 브라우저는 이후에
    // 결과물을 받아서 다시 역직렬화한 다음 렌더링을 수행 

    // 3. 브라우저가 리액트 컴포넌트 트리를 구성한다 브라우저가 서버로 스트리밍으로 JSON 결과물을 받았다면 이 구문을 다시 
    // 파싱한 결과물을 바탕으로 트리를 재구성해 컴포넌트를 만들어 나간다 최종적으로 이 트리를 렌더링해 브라우저의 DOM에 커밋한다

    // 서버 컴포넌트의 작동 방식의 특이점

    // 서버에서 클라이언트로 정보를 보낼 때 스트리밍 형태로 보냄으로써 클라이언트가 줄 단위로 JSON을 읽고 컴포넌트를 렌더링 할 수 있어
    // 브라우저에서는 되도록 빨리 사용자에게 결과물을 보여줄 수 있다는 장점이 있다

    // 컴포넌트들이 하나의 번들러 작업에 포함돼 있지 않고 각 컴포넌트별로 번들링이 별개로 돼 있어 필요에 따라 컴포넌트를 지연해서 받거나
    // 따로 받는 등의 작업이 가능해졌다

    // 서버 사이드 렌더링과는 다르게 결과물이 HTML이 아닌 JSON 형태로 보내진다
    // 클라이언트의 최종 목표는 리액트 컴포넌트 트리를 서버 컴포넌트와 클라이언트 컴포넌트의 두 가지로 조화롭게 구성하는 것 
    // 이는 단순히 HTML을 그리는 작업 이상의 일을 필요로 한다
    // 따라서 HTML 대신 단순한 리액트 컴포넌트 구조를 JSON으로 받아서 리액트 컴포넌트 트리의 구성을 최대한 빠르게 할 수 있도록 도와준다

    // 이러한 특징으로 인해 생기는 제약 사항은 서버 컴포넌트에서 클라이언트 컴포넌트로 props를 넘길 때 반드시 직렬화 가능한 데이터를 넘겨야 한다는 것이다
    // JSON으로 직렬화 할 수 없는 데이터 (class나 Date등)는 서버에서 클라이언트로 넘겨줄 방법이 없다
}
