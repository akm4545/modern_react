...

{
    ...
    // 클라이언트 컴포넌트에서 서버 액션을 쓰고 싶을 때는 앞의 startTransition 예제처럼 'use server'로 서버 액션만 모여 있는
    // 파일을 별도로 import 해야 한다

    // 서버 액션을 import 하는 것뿐만 아니라 props 형태로 서버 액션을 클라이언트 컴포넌트에 넘기는 것 또한 가능하다
    // 이는 서버 컴포넌트가 클라이언트 컴포넌트를 불러올 수 있는 것과 동일한 원리다
    // 즉 서버에서만 실행될 수 있는 자원은 반드시 파일 단위로 분리해야 한다
}

{
    // 그 밖에 프로젝트 전체 라우트에서 쓸 수 있는 미들웨어가 강화됐고 SEO(Search Engine Optimization)를 쉽게 작성할 수 있는 기능이
    // 추가됐으며 정적으로 내부 링크를 분석할 수 있는 기능 등 다양한 내용이 추가됐다
}