...

{
    ...
    // 리액트 파이버의 작업이 끝나면 리액트는 단순히 포인터만 변경해 workInProgress 트리를 현재 트리로 바꾸는데 이를 더블 버퍼링이라고 한다
    // 더블 버퍼링 = 컴퓨터 그래픽 분야에서 사용하는 용어
    // 그래픽을 통해 화면에 표시되는 것을 그리기 위해서는 내부적으로 처리를 거쳐야 한다
    // 이러한 처리를 거치게 되면 사용자에게 미처 다 그리지 못한 모습을 보는 경우가 발생
    // 이런 상황을 방지하기 위해 보이지 않는 곳에서 그다음으로 그려야 할 그림을 미리 그린 다음 이것이 완성되면 현재 상태를 새로운 그림으로 바꾸는 기법
    // 현재 UI 렌더링을 위해 존재하는 트리인 current를 기준으로 모든 작업이 시작
    // 만약 업데이트가 발생하면 바이버는 리액트에서 새로 받은 데이터로 새로운 workInProgress 트리를 빌드
    // 트리를 빌드하는 작업이 끝나면 다음 렌더링에 workInProgress트리를 사용
}

{
    // 파이버의 작업 순서
    // 1. 리액트는 beginWork() 함수를 실행해 파이버 작업을 수행하는데 더 이상 자식의 없는 파이버를 만날 때까지 트리 형식으로 시작 
    ...
    // 2. A1은 자식이 있으므로 B1로 이동해 beginWork()수행
    // 3. B1은 자식이 없으므로 completeWork()를 수행 자식이 없으므로 형제인 B2로 넘어간다
    // 4. B2의 beginWork()가 수행 자식이 있으므로 C1로 이동
    // 5. C1의 beginWork()가 수행 자식이 있으므로 D1로 이동
    // 6. D1의 beginWork()가 수행
    // 7. D1은 자식이 없으므로 completeWork()가 수행 자식이 없으므로 형제인 D2로 넘어간다
    // 8. D2는 자식이 없으므로 completeWork()가 수행
    // 9. D2는 자식도 더 이상의 형제도 없으므로 위로 이동해 D1, C1, B2 순으로 completeWork()를 호출
    // 10. B2는 형제인 B3으로 이동해 beginWork()수행
    // 11. B3의 completeWork()가 수행되면 반환해 상위로 타고 올라간다
    // 12. A1의 completeWork()가 수행
    // 13. 루트 노드가 완성되는 순간 최종적으로 commitWokr()가 수행되고 이 중에 변경 사항을 비교해 업데이트가 필요한 변경 사항이 DOM에 반영 

    // setState등으로 업데이트가 발생할 시 workInProgress트리를 다시 빌드 
    // 해당 빌드 과정은 위와 같다 
    // 최초 렌더링 시에는 모든 파이버를 새롭게 만들었지만 업데이트 발생 시 파이버가 존재하므로 되도록 새로 생성하지 않고 기존 파이버에서 업데이트 된
    // props를 받아 파이버 내부에서 처리
    // 이런 반복적인 재조정 작업 때마다 새롭게 파이버 자바스크립트 객체를 만드는 것은 리소스 낭비
    // 따라서 가급적 객체를 새로 만들기보다는 기존에 있는 객체를 재활용하기 위해 내부 속성값만 초기화하거나 바꾸는 형태로 트리를 업데이트
    // 해당 작업은 우선순위가 높은 업데이트가 오면 현재 업데이트 작업을 일시 중단하거나 새롭게 만들거나 폐기할 수도 있다
    // 작업 단위를 나누어 우선순위 할당도 가능하다
    // 해당 작업일 파이버 단위로 나눠서 수행한다
}

{
    // 파이버 = 리액트 컴포넌트와 1:1로 정보를 가지고 있음
    // 리액트 아키텍처 내부에서 비동기로 처리
    // 하지만 실제 브라우저 구조인 DOM에 반영은 동기적으로 일어나야 함, 처리하는 작업이 많아 화면에 불완전하게 표시될 수 있는 가능성이 높음
    // 그래서 메모리상에서 먼저 수행해서 최종적인 결과물만 실제 브라우저 DOM에 적용
    // 가상 DOM = 웹 애플리케이션에만 통용되는 개념
    // 리액트는 부라우저가 아닌 환경에서도 사용할 수 있기 때문에 파이버와 가상 DOM은 동일 개념이 아님
}

{
    // 가상 DOM은 단순히 빠르다는 이유로 만들어진 것이 아니다
    // 어떤 값이 바뀌었고 또 그 값에 의한 변경점이 어떤게 있는지를 리액트 내부의 파이버와 재조정자가 내부적인 알고리즘을 통해 관리해 줌으로써
    // 대규모 웹 애플리케이션을 효율적으로 유지보수하고 관리할 수 있다
    // 가상 DOM과 리액트의 핵심은 더욱 빠르게 반영하는 것이 아닌 바로 값으로 UI를 표현하는 것
}