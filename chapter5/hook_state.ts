...
{
    // Zustand는 특별히 많은 코드를 작성하지 않아도 빠르게 스토어를 만들고 사용할 수 있다는 큰 장점이 있다
    // 스토어를 만들고 파생된 값을 만드는 데 단 몇 줄의 코드면 충분하다
    // 리덕스 대비 확실히 구별되는 특징
    // 간단하고 빠르게 상태를 정의할 수 있어 상태 관리 입장에서 한결 가볍고 편리하다
    // Zuntand 라비르러리의 크기도 고작 2.9kB밖에 되지 않는다
    // 작은 크기 답게 내부 코드 역시 초보자들이 보기에도 간단하게 작성돼 있다
    // 타입스크립트 기반으로 작성돼 있기 때문에 별도의 설정이 필요 없다
    // 리덕스와 마찬가지로 미들웨어를 지원한다
    // create의 두 번째 인수로 원하는 미들웨어를 추가하면 되는데 스토어 데이터를 영구히 보존할 수 있는 persist, 복잡한 객체를
    // 관리하기 쉽게 도와주는 immer, 리덕스와 함께 사용할 수 있는 리덕스 미들웨어 등이 있다
    // 미들웨어를 사용해 상태를 sessionStorage에 추가로 저장하는 등의 기본적인 상태 관리 작동 외에 추가적인 작업 정의 가능
}

{
    // 위의 3개의 라비르러리 모두 다운로드 순위권도 상위에 포진해 있고 훌륭한 메인테이너를 위시로 유지보수되고 있으니 사용하는데 크게 어려움은
    // 없을 것으로 보인다
}